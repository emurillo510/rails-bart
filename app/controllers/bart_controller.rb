...
def index
  ...
  #
  #  *Arguments*
  #  - +cmd=count+ => Requests current count of trains active in the system (Required)
  #  - +key=<key>+ => 	API registration key (Required)
  # 
  #  *Returns* (in JSON format not XML)
  ...
  def get_bart_train_count
    ...
    bart_train_count(client_options)
  end
  ...
  def get_bart_elevator_status
    ...
    bart_elevator_status(client_options)
  end
  ...
  #
  #  *Arguments*
  #  - +cmd=etd+ => Requests current advisory information (Required)
  #  - +orig=<station>+ => Only get messages for specified station. Defaults to "all". (Optional) 
  #                      [Note: currently only "orig=all" or leaving the orig parameter off are supported, 
  #                      the BART system does not currently provide station specific BSA messages.]
  #  - +key=<key>+ =>  API registration key (Required)
  #  - +plat=<platform>	+ =>  This will limit results to a specific platform. Valid platforms depend on the station, but can range from 1-4. (Optional)
